...
export default function ScanResults({
  ...
  const fullyConnectedTags = tags.filter(
    (tag) => tag.status === TagStatus.CONNECTED
  ).length
  const totalTags = tags.length
  const percentComplete = Math.round((fullyConnectedTags / totalTags) * 100)

  // Count status types for summary
  const connectedCount = tags.filter(
    (tag) => tag.status === TagStatus.CONNECTED
  ).length
  const misconfiguredCount = tags.filter(
    (tag) => tag.status === TagStatus.MISCONFIGURED
  ).length
  const incompleteCount = tags.filter(
    (tag) => tag.status === TagStatus.INCOMPLETE
  ).length
  const notFoundCount = tags.filter(
    (tag) => tag.status === TagStatus.NOT_FOUND
  ).length
      ...
      <div className="mb-4 sm:mb-6">
        <div className="flex justify-between mb-1">
          <span className="text-xs font-medium text-blue-700">
            {fullyConnectedTags} of {totalTags} tags properly connected
          </span>
          <span className="text-xs font-medium text-blue-700">
            {percentComplete}%
          </span>
